...
function createCopingRebuildMaterial(poolGroup) {
  const source = getAllPoolCopingMeshes(poolGroup)?.[0]?.material || null;
  if (source?.clone) {
    const cloned = source.clone();
    cloned.transparent = false;
    cloned.opacity = 1;
...
function clearPoolSpaCopingMiter(poolGroup) {
  const grp = poolGroup?.userData?.spaCopingMiterGroup;
  if (grp?.parent) grp.parent.remove(grp);
  disposeObject3D(grp);
  if (poolGroup?.userData) poolGroup.userData.spaCopingMiterGroup = null;

  const segs = poolGroup?.userData?.copingSegments;
  if (segs && typeof segs === 'object') {
    Object.values(segs).forEach((mesh) => {
      if (mesh) mesh.visible = true;
    });
  }
}

function makeExtrudedTopShape(points, depth, material) {
  if (!Array.isArray(points) || points.length < 3 || !(depth > 1e-4)) return null;
  ...
  const mesh = new THREE.Mesh(geo, material?.clone ? material.clone() : material);
  mesh.castShadow = true;
  mesh.receiveShadow = true;
  mesh.userData.isCoping = true;
  return mesh;
}

function clampOpeningToSegment(openMin, openMax, segMin, segMax, minRemain = 0.05) {
  const a = Math.max(segMin + minRemain, Math.min(segMax - minRemain, openMin));
  const b = Math.max(segMin + minRemain, Math.min(segMax - minRemain, openMax));
  if (!(b > a + 1e-4)) return null;
  return [a, b];
}

function updateRectanglePoolSpaCopingMiter(poolGroup, spaGroup) {
  clearPoolSpaCopingMiter(poolGroup);

  const segs = poolGroup?.userData?.copingSegments;
  const snapSide = spaGroup?.userData?.snapSide || null;
  const connectedSide = ({ left: 'west', right: 'east', front: 'south', back: 'north' })[snapSide] || null;
  if (!segs || !connectedSide) return;
  if (!(segs.north && segs.south && segs.east && segs.west)) return;

  const margin = getSpaChannelMargin(spaGroup);
  if (!(margin > 0)) return;
  ...
  spaGroup.getWorldPosition(spaCenterWorld);
  const spaCenterLocal = poolGroup.worldToLocal(spaCenterWorld.clone());
  const spaLength = Math.max(0.01, spaGroup.userData?.spaLength || 0.01);
  const spaWidth = Math.max(0.01, spaGroup.userData?.spaWidth || 0.01);
  const margins = getSpaChannelMargins(spaGroup, poolGroup);

  const minX = spaCenterLocal.x - spaLength * 0.5 - (margins.minX || 0);
  const maxX = spaCenterLocal.x + spaLength * 0.5 + (margins.maxX || 0);
  const minY = spaCenterLocal.y - spaWidth * 0.5 - (margins.minY || 0);
  const maxY = spaCenterLocal.y + spaWidth * 0.5 + (margins.maxY || 0);

  const sourceMesh = segs[connectedSide];
  if (!sourceMesh?.geometry || !sourceMesh?.material) return;
  sourceMesh.geometry.computeBoundingBox();
  const bbox = sourceMesh.geometry.boundingBox;
  if (!bbox) return;

  const size = new THREE.Vector3();
  bbox.getSize(size);
  const pos = sourceMesh.position.clone();
  const depth = Math.max(0.001, size.z);
  const zBase = pos.z - depth * 0.5;
  const stripWidth = Math.max(0.001, (connectedSide === 'north' || connectedSide === 'south') ? size.y : size.x);

  let segMin, segMax, openMin, openMax;
  if (connectedSide === 'north' || connectedSide === 'south') {
    segMin = pos.x - size.x * 0.5;
    segMax = pos.x + size.x * 0.5;
    openMin = minX;
    openMax = maxX;
  } else {
    segMin = pos.y - size.y * 0.5;
    segMax = pos.y + size.y * 0.5;
    openMin = minY;
    openMax = maxY;
  }

  const clamped = clampOpeningToSegment(openMin, openMax, segMin, segMax, Math.max(0.05, stripWidth * 0.5));
  if (!clamped) return;
  openMin = clamped[0];
  openMax = clamped[1];

  const miterInset = Math.min(stripWidth, Math.max(0.02, (openMax - openMin) * 0.5 - 0.01));
  const group = new THREE.Group();
  group.name = 'PoolSpaCopingMiterGroup';

  const addMesh = (pts) => {
    const mesh = makeExtrudedTopShape(pts, depth, sourceMesh.material);
    if (!mesh) return;
    mesh.position.z = zBase;
    mesh.renderOrder = sourceMesh.renderOrder || 3;
    group.add(mesh);
  };

  if (connectedSide === 'south' || connectedSide === 'north') {
    const innerY = connectedSide === 'south' ? pos.y + size.y * 0.5 : pos.y - size.y * 0.5;
    const outerY = connectedSide === 'south' ? pos.y - size.y * 0.5 : pos.y + size.y * 0.5;

    const leftLen = openMin - segMin;
    if (leftLen > 0.02) {
      if (connectedSide === 'south') {
        addMesh([
          new THREE.Vector2(segMin, outerY),
          new THREE.Vector2(Math.max(segMin, openMin - miterInset), outerY),
          new THREE.Vector2(openMin, innerY),
          new THREE.Vector2(segMin, innerY)
        ]);
      } else {
        addMesh([
          new THREE.Vector2(segMin, innerY),
    ...
    const rightLen = segMax - openMax;
    if (rightLen > 0.02) {
      if (connectedSide === 'south') {
        addMesh([
          new THREE.Vector2(openMax, innerY),
          new THREE.Vector2(Math.min(segMax, openMax + miterInset), outerY),
          new THREE.Vector2(segMax, outerY),
          new THREE.Vector2(segMax, innerY)
        ]);
      } else {
        addMesh([
          new THREE.Vector2(openMax, innerY),
    ...
    const innerX = connectedSide === 'west' ? pos.x + size.x * 0.5 : pos.x - size.x * 0.5;
    const outerX = connectedSide === 'west' ? pos.x - size.x * 0.5 : pos.x + size.x * 0.5;

    const lowLen = openMin - segMin;
    if (lowLen > 0.02) {
      if (connectedSide === 'west') {
        addMesh([
          new THREE.Vector2(outerX, segMin),
          new THREE.Vector2(outerX, Math.max(segMin, openMin - miterInset)),
          new THREE.Vector2(innerX, openMin),
          new THREE.Vector2(innerX, segMin)
        ]);
      } else {
        addMesh([
          new THREE.Vector2(innerX, segMin),
    ...
    const highLen = segMax - openMax;
    if (highLen > 0.02) {
      if (connectedSide === 'west') {
        addMesh([
          new THREE.Vector2(innerX, openMax),
          new THREE.Vector2(outerX, Math.min(segMax, openMax + miterInset)),
          new THREE.Vector2(outerX, segMax),
          new THREE.Vector2(innerX, segMax)
        ]);
      } else {
        addMesh([
          new THREE.Vector2(innerX, openMax),
  ...
  poolGroup.add(group);
  if (poolGroup.userData) poolGroup.userData.spaCopingMiterGroup = group;
}
...
function updateSpaChannelMeshes(ground, poolGroup, spaGroup) {
  ...
  clearSpaChannelMeshes(ground);
  updateRectanglePoolSpaCopingMiter(poolGroup, spaGroup);
  const group = new THREE.Group();
